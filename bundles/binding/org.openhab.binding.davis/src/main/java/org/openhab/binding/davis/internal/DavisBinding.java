...
public class DavisBinding extends AbstractActiveBinding<DavisBindingProvider> implements ManagedService {
	...
	public void openPort() throws InitializationException {
		...
		try {
			logger.trace("getPortIdentifier called!");
			portIdentifier = CommPortIdentifier.getPortIdentifier(port);
			logger.trace("getPortIdentifier finished!");

			try {
				logger.trace("open called!");	
				serialPort = (SerialPort) portIdentifier.open("openhab", 3000);				
				logger.trace("open finished, setSerialParams called!");
				...
				logger.trace("Exception while openPort called!");
				e.printStackTrace();
			}
